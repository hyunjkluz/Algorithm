 ...
 * @문제 링크 : https://www.acmicpc.net/problem/15644 구슬 탈출3
 */

public class Main_15644 {
	public static int N, M;
	public static int[][] dir = new int[][] { { 0, 1 }, { 1, 0 }, { 0, -1 }, { -1, 0 } };
	public static char[] dirChar = new char[] { 'R', 'D', 'L', 'U' };
	...
	public static void main(String[] args) throws IOException {
		...
		bfs(arr, red, blue);
		System.out.println(POSSIBLE ? minTurn + "\n" + minRoute : -1);
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < dir.length; i++) {
				...
				if (isFin[0]) {
					...
					minRoute = getPoint.route.toString() + dirChar[i];
					POSSIBLE = true;
					return;
				}
				...
				if (!visited[newRed.x][newRed.y][newBlue.x][newBlue.y]) {
					Points newPoints = new Points(newRed, newBlue, getPoint.getTurnPlus1(), getPoint.route.toString());
					newPoints.addRoute(dirChar[i]);
